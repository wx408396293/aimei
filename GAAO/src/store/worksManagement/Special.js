...
export const DELETESPECIAL = "DELETESPECIAL"
const Special={
	namespaced: true,
	state:{
	...
	actions:{
		...
		async WORKSLIST(context) {
    		...
    		var {data} = await axios({
				method:"post",
				baseURL: `${urls.getTopicPoolList}`,
			    // baseURL: `${context.state.url}/works/getTopicPoolList`,
			    timeout: 4000,
		...
		//删除专题
		async DELETESPECIAL(context,ids){
			let datas= new FormData()
    		datas.append('ids', ids);
			const {data} = await axios({
				method:"post",
				baseURL:  `${urls.removeFromTopicPool}`,
			    // baseURL: `${context.state.url}/works/removeFromTopicPool`,
			    timeout: 4000,
			    headers: {'cms-token': localStorage.token,'cms-channel' :'0'},
			    data:datas
			});

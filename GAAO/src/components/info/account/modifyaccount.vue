					...
					<el-form :model="form" :ref="form" :rules="rules1" ref="form"  label-width="80px" class="demo-ruleForm">
						<el-form-item label="账户名" prop="userName">
			  			...
			  			<el-form-item label="密码">
			    			<el-input size="small" type="password" :maxlength="16"  placeholder="不修改则留空" style="width: 210px;" v-model="form.password"></el-input>
			  			</el-form-item>
						...
						<el-button type="primary" :plain="true" @click="modify(form)">提交</el-button>
						<el-button type="danger" @click="resetLid">取消</el-button>
					</el-form>
		    	</el-col>
		    	</el-col :span="16">
					<p>头像</p>
...
<script>
	...
	import {DEPARTMENT,SETACCOUNT,DIALOGVIDIBLE,REVISEACCOUNTIST,setGATTACCOUNT} from "../../../store/account/account.js"
	export default{
		name:"addAccount",
		created(){
			this.form.userId = this.Gataccount.userId
			this.form.userName = this.Gataccount.userName
			this.form.resources = this.Gataccount.resources
			this.form.department = this.Gataccount.department
			this.form.roles = this.Gataccount.roles
			this.form.realName = this.Gataccount.realName
			this.form.gender = this.Gataccount.gender
			this.form.email = this.Gataccount.email
			this.form.phoneNumber = this.Gataccount.phoneNumber
			this.form.avatarUrl = this.Gataccount.avatarUrl
			//对应二级权限
			this.form.roles.split(",").forEach( (item) => {
				if (item === "2"){ this.jies="111"}
			    if (item === "3") {this.resets[0]= "3"}
				if (item === "4") {this.resets[1]= "4"}
				if (item === "5") this.resets[2]= "5"
				if (item === "6") this.resets[3]= "6"
				if (item === "7") this.resets[4]= "7"
				if (item === "8") this.resets[5]= "8"
				if (item === "9") this.resets[6]= "9"
				if (item === "10") this.resets[7]= "10"
				if (item === "11") this.resets[8]= "11"
				if (item === "12") this.resets[9]= "12"
				if (item === "13") this.resets[10]= "13"
				if (item === "14") this.resets[11]= "14"
				if (item === "15") this.resets[12]= "15"
				if (item === "16") this.resets[13]= "16"
				if (item === "17") this.resets[28]= "17"
				if (item === "18") this.resets[29]= "18"
				if (item === "19") this.resets[30]= "19"
				if (item === "20") this.resets[31]= "20"
				if (item === "21") this.resets[34]= "21"
				if (item === "22") this.resets[21]= "22"
				if (item === "23") this.resets[22]= "23"
				if (item === "24") this.resets[32]= "24"
				if (item === "25") this.resets[33]= "25"
				if (item === "26") this.resets[14]= "26"
				if (item === "27") this.resets[15]= "27"
				if (item === "28") this.resets[16]= "28"
				if (item === "29") this.resets[17]= "29"
				if (item === "30") this.resets[18]= "30"
				if (item === "31") this.resets[19]= "31"
				if (item === "32") this.resets[20]= "32"
				if (item === "33") this.resets[36]= "33"
				if (item === "34") this.resets[37]= "3"
				if (item === "35") this.resets[23]= "35"
				if (item === "36") this.resets[24]= "36"
				if (item === "37") this.resets[25]= "37"
				if (item === "38") this.resets[26]= "38"
				if (item === "39") this.resets[27]= "39"
				if (item === "40") this.resets[35]= "40"
				if (item === "41") this.resets[38]= "41"
				if (item === "42") this.resets[39]= "42"
			});
			//对应三级权限
			this.form.resources.split(",").forEach( (item) => {
				if (item === "2") this.resourcesList[0]= "2"
				if (item === "3") this.resourcesList[1]= "3"
				if (item === "4") this.resourcesList[2]= "4"
				if (item === "5") this.resourcesList[3]= "5"
				if (item === "6") this.resourcesList[4]= "6"
				if (item === "7") this.resourcesList[5]= "7"
				if (item === "8") this.resourcesList[6]= "8"
				if (item === "9") this.resourcesList[7]= "9"
				if (item === "10") this.resourcesList[8]= "10"
				if (item === "11") this.resourcesList[9]= "11"
				if (item === "12") this.resourcesList[10]= "12"
				if (item === "13") this.resourcesList[11]= "13"
				if (item === "14") this.resourcesList[12]= "14"
				if (item === "15") this.resourcesList[28]= "15"
				if (item === "16") this.resourcesList[29]= "16"
				if (item === "17") this.resourcesList[13]= "17"
				if (item === "18") this.resourcesList[14]= "18"
				if (item === "19") this.resourcesList[15]= "19"
				if (item === "20") this.resourcesList[16]= "20"
				if (item === "21") this.resourcesList[18]= "21"
				if (item === "22") this.resourcesList[19]= "22"
				if (item === "23") this.resourcesList[20]= "23"
				if (item === "24") this.resourcesList[21]= "24"
				if (item === "25") this.resourcesList[22]= "25"
				if (item === "26") this.resourcesList[17]= "26"
				if (item === "27") this.resourcesList[23]= "27"
				if (item === "28") this.resourcesList[24]= "28"
				if (item === "29") this.resourcesList[25]= "29"
				if (item === "30") this.resourcesList[26]= "30"
				if (item === "31") this.resourcesList[27]= "31"
			});
		...
		methods:{
				...
				this.$refs[form].validate((valid) => {
		            if (valid) {
						...
						if (this.form.roles === "") {
							...
							this.REVISEACCOUNTIST(this.form)
						}
		            } else {
		            	return false;
		            }
		    ...
		    ...mapMutations("account",["DIALOGVIDIBLE","setGATTACCOUNT"]),
		    ...mapActions("account",["DEPARTMENT","SETACCOUNT","REVISEACCOUNTIST"]),
		}
	}
</script>
<style scoped>

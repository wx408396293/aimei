			    	...
			    	<el-table
					    ...
					    <el-table-column label="操作" width="100" fixed="right">
					        <template scope="scope">
						         <el-button
						          size="small"
						          @click="worksAddSelecteS(scope.row)">添加</el-button>
					        </template>
					    </el-table-column>
					</el-table>
		    	</el-col>
...
<script>
	...
	import {VIDEOLIST,APPSPECIAL,APPSELECTED} from "../../../../store/worksManagement/Special.js"
	export default{
		name:"worksList",
		created(){
			this.VIDEOLIST()
		...
		computed:{
	    	...mapState("works",["videoSiez","videoElements","VideoINdex","VidoeList"]),
	    	...mapState("addSelecteds",["SelectedsLIST"]),
	    },
		methods:{
			...mapActions("works",["VIDEOLIST",]),
			...mapMutations("works",["getvideoSize"]),
			...mapMutations("addSelecteds",["getSelecteds"]),
	        //翻页
        	handleCurrentChange(val) {
        		
        		this.getvideoSize(val)
        		this.VIDEOLIST()
        		this.currentPage = this.VideoINdex
        	},
        	//查询
        	QueryCriteria(){
        		...
        		if (this.SelectedsLIST.length !== 0) {
        			...
        			if (buer) {
    					...
    					if (this.SelectedsLIST.length ===10) {
    						this.$message({
					          showClose: true,
					          message: '最多能添加10条数据。请关闭作品页面',
					          type: 'warning'
					        });
    					}else {
    						this.getSelecteds(row)
    					}
    				}

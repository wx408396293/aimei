	    ...
	    <div class="div">
			<el-row>
		    	<el-col :span="24">
					...
					<div class="button">
						<el-button size="small"  @click="SubmitButton">修改</el-button>
						<el-button size="small"  @click="setButton">取消</el-button>
					</div>
...
import {mapMutations,mapActions, mapState} from "vuex"
	export default{
		...
		created(){
			this.getFrom.id = this.notification.id
			this.from.content = this.notification.content
			this.from.date =this.notification.date
			this.from.UserName = this.notification.targetIdSets
			if (this.notification.targetIdSets ==="指定接收用户"){
				this.displaytextarea =true
				this.from.userId =  this.notification.targetIds
			}
			//0=视频 1=音频 2=专题 3=专辑 4=榜单 5=标签详情 6=素材详情 7=个人主页 8=活动详情 9=链接
			switch(this.notification.shareContent.type) {
			   	case "视频": 
			   		this.from.type = "视频"
			   		this.from.getId = this.notification.shareContent.sourceId
		...
		computed:{
	    	...mapState("notificationlist",["notification"])
	    },
		methods:{
			...mapActions("notificationlist",["MODIFYNOTIFICATION"]),
			...mapMutations("notificationlist",["setnotifications"]),
			...
			SubmitButton(){
				...
				this.MODIFYNOTIFICATION(this.getFrom)
			}
		}
	}
</script>

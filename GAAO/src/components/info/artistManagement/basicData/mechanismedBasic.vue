					...
					<div class="cantent">
						<div class="namens">
							<div class="names_maix">
								<label>机构名称</label>
								<span>{{audite_A.enterpriseName}}</span>
							</div>
							<div class="names_maix">
								<label>营业执照注册号</label>
								<span>{{audite_A.registerNumber}}</span>
							</div>
							<div class="names_maix">
								<label>社会信用代码</label>
								<span>{{audite_A.creditCode}}</span>
							</div>
						</div>
						<div class="namens">
							<div class="names_maix">
								<label>法人/企业负责人</label>
								<span>{{audite_A.legalPerson}}</span>
							</div>
							<div class="names_maix">
								<label>经营范围</label>
								<span>{{audite_A.scopeOfBusiness}}</span>
							</div>
							<div class="names_maix">
								<label>联系方式</label>
								<span>{{audite_A.contactWay}}</span>
							</div>
						</div>
						<div class="namens">
							<div class="names_maix">
								<label>证件照片</label>
								<img :src="audite_A.license" >
...
import router from "@/router/index.js"
	export default{
		...
		computed:{
	      ...mapState("mechanismList",["audite_A","jumphomepage"]),
	      RevokWhy:{
	      	get(){
	      		return this.$store.state.mechanismList.RevokWhy
	      	},
	      	set(){}
	      },
	    },
		methods:{
			...mapActions("mechanismList",["REVOKAUDITED"]),
			...mapMutations("mechanismList",["getRevokWhy"]),
	        //撤销
	        RevokeButon(){
	        	this.getRevokWhy()
	        },
